...
class Advisor_all_custom_ae(BaseAgent):
    def __init__(self, nf, sess, *args, **kwargs):
        super(Advisor_all_custom_ae, self).__init__(*args, **kwargs)
    ...
    def advisor_used(self):
        return self.last_advisor
    ...
    '''
    Gets the recommended actions from each advisor in a given state
    '''
    def get_advisor_actions(self, obs):
        ...
        # pick a purely random direction
            ...
            unsafe_directions = self._filter_safe_directions(board, my_position,
                                                        directions_all, bombs)
            # print(f'unsafe directions: {unsafe_directions}')
            if len(unsafe_directions) == 0:
                unsafe_directions = directions_all
            if advisor_actions[0] == -1:
                advisor_actions[0] = random.choice(unsafe_directions).value
            # print(f'direction chosen from: {unsafe_directions}')

        # Advisor 1 - right expert
        if my_position[0] > 5:
            # Move if we are in an unsafe place.
            unsafe_directions = self._directions_in_range_of_bomb(
                board, my_position, bombs, dist)
        ...
        advisor_actions[2] = random.choice(directions_all).value
    ...
    @staticmethod
    def _filter_safe_directions(board, my_position, directions, bombs):
        ret = []
        for direction in directions:
            x, y = utility.get_next_position(my_position, direction)
            is_bad = True
